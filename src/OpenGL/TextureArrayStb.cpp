#include "TextureArrayStb.h"

#include "stb_image.h"

namespace OGL
{
	TextureArrayStb::TextureArrayStb(const std::string* paths, const unsigned int amount)
	...
	TextureArrayStb::~TextureArrayStb()
	{
		GlCall(glDeleteTextures(1, &m_RendererID));
	}

	void TextureArrayStb::Bind(unsigned int slot) const
	{
		GlCall(glActiveTexture(GL_TEXTURE0 + slot));
		GlCall(glBindTexture(GL_TEXTURE_2D_ARRAY, m_RendererID));
	}

	void TextureArrayStb::Unbind()
	{
		GlCall(glBindTexture(GL_TEXTURE_2D_ARRAY, 0));
	}
}
